...
from PySide6.QtWidgets import (
    QWidget, QFormLayout, QSpinBox, QDoubleSpinBox, QCheckBox, QLineEdit
)
    ...
    def rebuild_params_form(self, name: str):
        ...
        for p in spec.params:
            if p.ptype == "int":
                ...
                w = QDoubleSpinBox()
                w.setDecimals(3)
                if p.min_val is not None: w.setMinimum(float(p.min_val))
                if p.max_val is not None: w.setMaximum(float(p.max_val))
